...
function ricerca2(event) {
    ...
    const url2=splitUrl[0]+'//'+splitUrl[2]+'/'+splitUrl[3]+'/api_nutritionix.php?query=';
    const url3 = url2 + query;
    fetch(url3).then(onResponse2).then(onjson2);
}
...
function content2(json) {
    Dottori= json;
   
}
const url=window.location.href;
var splitUrl = url.split('/');
url2=splitUrl[0]+'//'+splitUrl[2]+'/'+splitUrl[3]+'/fetch_service.php'; 
fetch(url2).then(onResponse).then(content);
var Servizi='';
const url3=window.location.href;
var splitUrl = url3.split('/');
url2=splitUrl[0]+'//'+splitUrl[2]+'/'+splitUrl[3]+'/fetch_doctor.php';
fetch(url2).then(onResponse).then(content2);
var Dottori='';
